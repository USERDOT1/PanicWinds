...
@onready var projectile = load("res://Scenes/homing_rocket.tscn")
...
func action(delta):
	if GlobalVars.roundStart <= 0:
		if GlobalVars.missiles > 0:
			...
			GameManager.spawnProjectile.rpc("res://Scenes/homing_rocket.tscn", playerOwner.rotation,self.rotation,self.global_position,GlobalVars.xVelocity,GlobalVars.yVelocity, playerOwner.name, 35 * GlobalVars.shipDamageMultiplyer,0)
		else:
			$NoMissiles.play()
