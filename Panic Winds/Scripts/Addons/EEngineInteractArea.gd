...
func _process(_delta): #add flames
	if $"..".playerOwner == GlobalVars.currentPlayer:
		...
		if $"..".InputBind == "InputTwo" && !done:
			if Input.is_action_pressed(GlobalVars.twoBind):
				if GlobalVars.Energy > 1:
		...
		if $"..".InputBind == "InputThree" && !done:
			if Input.is_action_pressed(GlobalVars.threeBind):
				if GlobalVars.Energy > 1:
		...
		if $"..".InputBind == "InputFour" && !done:
			if Input.is_action_pressed(GlobalVars.fourBind):
				if GlobalVars.Energy > 1:
					$"../EngineSprite".animation = "EngineBindFourActivae"
				done = true
			else:
				$"../EngineSprite".animation = "EngineBindFour"
		if $"..".InputBind == "InputFive" && !done:
			if Input.is_action_pressed(GlobalVars.fiveBind):
				if GlobalVars.Energy > 1:
		...
		if $"..".InputBind == "InputSix" && !done:
			if Input.is_action_pressed(GlobalVars.sixBind):
				if GlobalVars.Energy > 1:
		...
		if $"..".InputBind == "InputSeven" && !done:
			if Input.is_action_pressed(GlobalVars.sevenBind):
				if GlobalVars.Energy > 1:
		...
		if $"..".InputBind == "InputEight" && !done:
			if Input.is_action_pressed(GlobalVars.eightBind):
				if GlobalVars.Energy > 1:
		...
		if $"..".InputBind == "InputNine" && !done:
			if Input.is_action_pressed(GlobalVars.nineBind):
				if GlobalVars.Energy > 1:
